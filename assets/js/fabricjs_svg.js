...
fabric.Object.prototype.id = '';
fabric.Object.prototype.awal = '';
fabric.Object.prototype.zoom = 0;
fabric.Object.prototype.stacked = false;
fabric.Object.prototype.groupName = false;
fabric.Object.prototype.lock = false;
//canvas 
fabric.Canvas.prototype.canvasScale = 1;
//static canvas
fabric.StaticCanvas.prototype.pointer = null;
...
fabric.ElementsParser.prototype._createObject = function (klass, el, index) {
  ...
  opt.clip = null;
  console.log(el.parentNode.style['clip-path']);
  if (el.parentNode.tagName == 'g' && el.parentNode.style['clip-path'] != '' && el.parentNode.style['clip-path'] != null) {
    var root = Root(el);
    var cp = root.getElementById(el.parentNode.style['clip-path'].replace('url(#', '').replace(')', ''));
...
fabric.parseSVGDocument = function () {
  var reAllowedSVGTagNames = /^(path|circle|polygon|polyline|ellipse|rect|line|image|text)$/,
    ...
    reNum = '(?:[-+]?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)', reViewBoxAttrValue = new RegExp('^' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*' + '$');
  ...
  return function (doc, callback, reviver) {
    ...
    if (descendants.length === 0) {
      ...
      var arr = [];
      var i; for(i = 0, len = descendants.length; i < len; i++) {
        arr[i] = descendants[i];
      }
      descendants = arr;
    }
...
function getGBound(o) {
  var area = getArea(o);
  var minX = fabric.util.array.min(area.ax) + fabric.util.array.min(area.bx), minY = fabric.util.array.min(area.ay) + fabric.util.array.min(area.by), maxX = fabric.util.array.max(area.ax) + fabric.util.array.max(area.bx), maxY = fabric.util.array.max(area.ay) + fabric.util.array.max(area.by);
  return {
    minX: minX,
    minY: minY,
    maxX: maxX,
...
fabric.Canvas.prototype.findById = function (id) {
  var i; for(i in this.getObjects()) {
...
function loadSVG(mode, c, fn) {
  currentCanvasId = c.id;
  fabric.loadSVGFromURL('/api/templates/' + param.template + '_' + mode + '.svg', function (objects, options) {
    var gn = '';
    var group = [];
    ...
    var i; for(i in objects) {
      ...
      if (gn != o.groupName) {
        if (group.length > 0) {
          ...
          if (param.nocrop != '1' && cek(group[0], 'groupName') != 'layerone') {
    ...
    c.remove(g);
    var i=0;for (i = 0; i < items.length; i++) {
      items[i].setCoords();
      if (items[i].groupName == 'layerone') {
        c.addLater(items[i]);
      } else {
        c.add(items[i]);
        //stateupdate(1);
